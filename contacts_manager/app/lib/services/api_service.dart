...
class ApiService {
  // host is chosen per platform (localhost on desktop/web, 10.0.2.2 on Android emulator)
  ...
  static Future<ApiResponse<List<Contact>>> getAllContacts() async {
    try {
      final response = await http
          .get(Uri.parse('${baseUrl}$contactsEndpoint'), headers: _headers)
          .timeout(timeout);
  ...
  static Future<ApiResponse<Contact>> getContactById(String id) async {
    try {
      final response = await http
          .get(Uri.parse('${baseUrl}$contactsEndpoint/$id'), headers: _headers)
          .timeout(timeout);
  ...
  static Future<ApiResponse<Contact>> createContact(Contact contact) async {
    try {
      final response = await http
          .post(Uri.parse('${baseUrl}$contactsEndpoint'), headers: _headers, body: json.encode({
        'nome': contact.nome,
  ...
  static Future<ApiResponse<Contact>> updateContact(Contact contact) async {
    try {
      final response = await http
          .put(Uri.parse('${baseUrl}$contactsEndpoint/${contact.id}'), headers: _headers, body: json.encode({
        'nome': contact.nome,
        'telefone': contact.telefone,
        'email': contact.email,
      })).timeout(timeout);
  ...
  static Future<ApiResponse<Contact>> deleteContact(String id) async {
    try {
      final response = await http
          .delete(Uri.parse('${baseUrl}$contactsEndpoint/$id'), headers: _headers)
          .timeout(timeout);
